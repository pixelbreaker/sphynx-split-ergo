...
export class Alignable extends Shape3 {
  ...
  align(v: Vec3) {
    const src = this.translate(
      v.map((a, i) => a * (this.size[i] / 2)) as Vec3
    ).src;
...
const round = (
  ...
  const [c1, ...corners] = Array.from(new Array(Math.pow(2, dim))).map(
    (_, i) => {
      const r = Math.max(radii[i % radii.length], 0.001);
      const p = Array.from(new Array(dim)).map((_, j) =>
        (i & (1 << j)) > 0 ? half[j] - r : -half[j] + r
      );
...
export const square = (p: Vec2) =>
  new Square(p, [`square(size=${serialize(p)}, center=true);`]);
...
export const sector = (angle: number, radius: number = 1000): Shape2 => {
  ...
  const points = Array.from(new Array(divisions + 1)).map(
    (_, i) =>
      [
...
export const polyhedron = (p: PolyhedronProps) =>
  shape3([`polyhedron(${serialize(p)});`], p);
